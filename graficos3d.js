// ==========================================
// MOTOR DE OTIMIZAÇÃO (RADAR 3D) - BLINDADO PARA LAZY LOAD
// ==========================================
window.RadarDePerformance = {
    elementosVisiveis: new Set(),
    elementosRastreados: new Set(), // Nova memória para elementos dinâmicos
    observer: null,
    
    iniciar: () => {
        window.RadarDePerformance.observer = new IntersectionObserver((entries) => {
        ...
        // As listas manuais foram removidas. O Radar agora é autônomo.
    },

    podeAnimar: (id) => {
        // A MÁGICA: Se o módulo (como o Jardim) foi injetado agora e o Radar não conhece, ele rastreia na hora!
        if (!window.RadarDePerformance.elementosRastreados.has(id)) {
            const el = document.getElementById(id);
            if (el && window.RadarDePerformance.observer) {
                window.RadarDePerformance.observer.observe(el);
                window.RadarDePerformance.elementosRastreados.add(id);
                // Libera a placa de vídeo no primeiro milissegundo para a tela não piscar
                return true; 
            }
            return false;
        }
        // Se já está rastreado, obedece rigorosamente ao olho do radar para poupar bateria
        return window.RadarDePerformance.elementosVisiveis.has(id);
    },

    // 🚀 NOVO: Motor de Limpeza Absoluta do Three.js (Destrói Geometrias e Materiais da VRAM)
    limparCena3D: (scene, renderer) => {
        if (!scene || !renderer) return;
        
        scene.traverse((object) => {
            if (!object.isMesh) return;
            
            // Destrói as Malhas (Geometrias)
            if (object.geometry) {
                object.geometry.dispose();
            }
            
            // Destrói os Materiais (Cores, Texturas, Reflexos)
            if (object.material) {
        ...
        // Limpa o renderizador da memória
        renderer.dispose();
    }
};
...
// ==========================================
window.otimizadorDeResize = function(func, delay = 100) {
    let timeout;
    return function() {
...
window.inicializarOrbeClima = () => {
    ...
    // 🚨 LIMPEZA ABSOLUTA DE VRAM: Se já existe um Orbe oculto, destrua-o completamente da memória!
    if (window.OrbeClimaScene && window.OrbeClimaRenderer) {
        window.RadarDePerformance.limparCena3D(window.OrbeClimaScene, window.OrbeClimaRenderer);
    }
    container.innerHTML = "";

    const scene = new THREE.Scene();
    window.OrbeClimaScene = scene; // Salva a referência global para limpeza futura
    ...
    window.OrbeClimaRenderer = renderer; // Salva a referência global
...
// ==========================================
// ... (A sua implementação original está perfeita e não usa WebGL, usa Canvas 2D, que não sofre VRAM Leak! Nenhuma mudança profunda necessária) ...
...
window.iniciarGravacao = async () => { /* Sua lógica perfeita */ };
window.pararGravacao = () => { /* Sua lógica perfeita */ };
window.tocarEco = async () => { /* Sua lógica perfeita */ };
...
// --- 2. A ÁRVORE DA VIDA (FRACTAL PROCEDURAL BLINDADO) ---
window.inicializarPrisma3D = () => {
    const container = document.getElementById('prisma-3d');
    if (!container || typeof THREE === 'undefined') return;

    // 🚨 LIMPEZA ABSOLUTA DE VRAM
    if (window.PrismaScene && window.PrismaRenderer) {
        window.RadarDePerformance.limparCena3D(window.PrismaScene, window.PrismaRenderer);
    }
    container.innerHTML = ''; 

    const scene = new THREE.Scene();
    window.PrismaScene = scene;
    
    // Força a leitura inicial. Se o container estiver com 0px (escondido), assumimos um mínimo seguro.
    ...
    window.PrismaRenderer = renderer;
    ...
    // Injeta a placa de vídeo na tela
    container.appendChild(renderer.domElement);
    ...
    // O Radar de Performance continua operante
    ...
    const animar = (tempoAtual) => {
        ...
        // Hibernação: Só anima se a aba estiver na tela e o Radar liberar
...
window.inicializarPlanetario3D = () => {
    ...
    // 🚨 CORREÇÃO SUPREMA: O Observador de Tamanho!
    // Ele avisa o Canvas para se redesenhar no exato momento que o modal abrir
    const redimensionar = () => {
        const rect = container.getBoundingClientRect();
    ...
    let cameraX = 0, cameraY = 0, isDragging = false, lastX = 0, lastY = 0;
    let velX = VELOCIDADE_ROTACAO_X, velY = VELOCIDADE_ROTACAO_Y;
    ...
    const onPointerUp = () => { isDragging = false; wrapper.style.cursor = 'grab'; };
    ...
    const animarPlanetario = () => {
        ...
        const largura = canvas.width, altura = canvas.height, dpr = window.devicePixelRatio || 1;
...
window.inicializarGalaxia3D = () => {
    ...
    // 🚨 LIMPEZA ABSOLUTA DE VRAM
    if (window.GalaxiaScene && window.GalaxiaRenderer) {
        window.RadarDePerformance.limparCena3D(window.GalaxiaScene, window.GalaxiaRenderer);
    }
    container.innerHTML = '';

    const scene = new THREE.Scene();
    window.GalaxiaScene = scene;
    ...
    window.GalaxiaRenderer = renderer;
    ...
    let tempo = 0;
    const animar = () => {
        if (!window.GalaxiaRenderer) return;
...
window.inicializarJornada3D = () => {
    ...
    // 🚨 LIMPEZA ABSOLUTA DE VRAM
    if (window.JornadaScene && window.JornadaRenderer) {
        window.RadarDePerformance.limparCena3D(window.JornadaScene, window.JornadaRenderer);
    }
    container.innerHTML = '';

    const scene = new THREE.Scene();
    window.JornadaScene = scene;
    ...
    window.JornadaRenderer = renderer;
